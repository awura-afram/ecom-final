...
// print_r($decodedResponse->data->plan_object);
// echo $decodedResponse->data->name;
// print_r($decodedResponse->data->plan_object->name);
...
if (isset($decodedResponse->data->status) && $decodedResponse->data->status == 'success') {
    ...
    $sub_type = $decodedResponse->data->plan_object->name;
    ...
    if ($add_order) {
        ...
        if ($add_Payment) {
            ...
            // add sub
            $add_sub = add_subscription_ctr($user_id, $sub_type, $order_status);

            if ($add_sub) {
                header("Location: ../view/meal_plan.php?message=$sub_type");
            }
        } else {
            echo "Payment Failed";
        }
